/***********************************************
	...
	문제 : iterative 버전으로 AVL Tree insert 알고리즘을 구현하여라 

	구현내용 : 메인함수에서 inputdata로 부터 파일을 읽어온 후, avl 트리구조로 프로그램을 만든다.
			  삽입이 끝나면 종료(4번)을 누르고, 결과값으로 data value와 balance 값을 확인할 수있다.
	만든날짜 : 2012. 10.29
***********************************************/
#include <stdio.h>
#include <stdlib.h>
...
void display(avl_node *p){
	...
	if(p!=NULL){
		...
		printf("[data value] :%s [balance value] :(%d) \n",p->key.p_num,get_height(p->left_child) - get_height(p->right_child));
